## Score TCGA PRAD patients using 2018 GOBP
...
## filter PRAD iso data to just genes in pathways
## load(file = "~/Dropbox/Splice-n-of-1-pathways/Data/prad_iso_paired.RData")
## prad_iso_gobp_data <- prad_iso[which(prad_iso$geneSymbol %in% genes), ]
## str(prad_iso_gobp_data)
## tmp_genes <- unique(as.character(gobp$symbol))
## saveRDS(prad_iso_gobp_data, file = "~/Dropbox/Splice-n-of-1-pathways/Data/prad_iso_gobp_data.rds")

## load TCGA PRAD TPM isoform data
prad_iso_gobp_data <- readRDS(file = "~/Dropbox/Splice-n-of-1-pathways/Data/prad_iso_gobp_data.rds") 
## DEG isoform data
load(file = "~/Dropbox/Splice-n-of-1-pathways/Data/prad_deg_list_all.RData") 
...
pat_col <- grep("TCGA", x = names(prad_iso_gobp_data))
patients_chr <- unique(substring(names(prad_iso_gobp_data[pat_col]), 1, 12))
...
for (tmp_pat in patients_chr) {
    ...
    iso_gobp_list[[tmp_pat]] <- (data.frame(geneSymbol = prad_iso_gobp_data$geneSymbol,
                                            prad_iso_gobp_data[, grep(tmp_pat, names(prad_iso_gobp_data))]))
    iso_gobp_list[[tmp_pat]][, "geneSymbol"] <- as.character(prad_iso_gobp_data$geneSymbol)
}

##############################################################################
#### 2b. Restructure DEG data into a patient-wise list for parallel processing

## DEG_dat <- prad_deg_list[[1]]
...
DEG_list <- lapply(prad_deg_list_all, get_DEGs, threshold=0.2)
...
## str(scores_list)
## save the object
save(scores_list, file = "~/Dropbox/Splice-n-of-1-pathways/Data/TCGA_PRAD_hel_EE_Iso30_exprThreshold=5_NoDEG_GOBP_9aug2018.RData")
