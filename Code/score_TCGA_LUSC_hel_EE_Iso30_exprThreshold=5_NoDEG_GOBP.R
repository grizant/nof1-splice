## Score TCGA LUSC patients using 2018 GOBP
...
## filter LUSC iso data to just genes in pathways
## load(file = "~/Dropbox/Splice-n-of-1-pathways/Data/lusc_iso_paired.RData")
## lusc_iso_gobp_data <- lusc_iso[which(lusc_iso$geneSymbol %in% genes), ]
## str(lusc_iso_gobp_data)
## tmp_genes <- unique(as.character(gobp$symbol))
## saveRDS(lusc_iso_gobp_data, file = "~/Dropbox/Splice-n-of-1-pathways/Data/lusc_iso_gobp_data.rds")

## load TCGA LUSC TPM isoform data
lusc_iso_gobp_data <- readRDS(file = "~/Dropbox/Splice-n-of-1-pathways/Data/lusc_iso_gobp_data.rds") 
## DEG isoform data
...
pat_col <- grep("TCGA", x = names(lusc_iso_gobp_data))
patients_chr <- unique(substring(names(lusc_iso_gobp_data[pat_col]), 1, 12))
...
for (tmp_pat in patients_chr) {
    ...
    iso_gobp_list[[tmp_pat]] <- (data.frame(geneSymbol = lusc_iso_gobp_data$geneSymbol,
                                            lusc_iso_gobp_data[, grep(tmp_pat, names(lusc_iso_gobp_data))]))
    iso_gobp_list[[tmp_pat]][, "geneSymbol"] <- as.character(lusc_iso_gobp_data$geneSymbol)
}
...
## str(scores_list)
## save the object
save(scores_list, file = "~/Dropbox/Splice-n-of-1-pathways/Data/TCGA_LUSC_hel_EE_Iso30_exprThreshold=5_NoDEG_GOBP_9aug2018.RData")
