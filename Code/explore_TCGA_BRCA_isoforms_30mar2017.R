...
library(data.table)
iso_data <- fread(file = "Total_FPKM_paired.txt", sep = "\t", header = T, stringsAsFactors = F)

## check normalization
## colSums(iso_data[,-(1:4)])
...
iso_go_data <- iso_data[iso_data$gene_short_name %in% go_genes,]
nrow(iso_go_data)
nrow(iso_data)
...
save(example_iso_data, file = "~/Dropbox/Splice-n-of-1-pathways/Data/example_iso_data.RData")

## get distribution of the number of alternative spliced proteins
system.time(gene_go_data <- split(iso_go_data, iso_go_data$gene_short_name)) ## 25 seconds

length(gene_go_data) ## 11,435 unique genes
...
qplot(gene_splice_count, tmp_hel_dist)
