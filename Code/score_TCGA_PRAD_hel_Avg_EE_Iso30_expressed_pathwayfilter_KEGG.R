## Score TCGA PRAD patients via Average pathway Hellinger distances
...
load(file = "~/Dropbox/Splice-n-of-1-pathways/Data/prad_iso_kegg_data.RData") ## NEW ISOFORM 25 Jul 2017
...
pat_col <- grep("TCGA", x = names(prad_iso_kegg_data))
patients_chr <- unique(substring(names(prad_iso_kegg_data[pat_col]), 1, 12))
...
for (tmp_pat in patients_chr) {
    ...
    iso_kegg_list[[tmp_pat]] <- (data.frame(geneSymbol = prad_iso_kegg_data$geneSymbol,
                                            prad_iso_kegg_data[, grep(tmp_pat, names(prad_iso_kegg_data))]))
    iso_kegg_list[[tmp_pat]][, "geneSymbol"] <- as.character(prad_iso_kegg_data$geneSymbol)
}
...
## 
## ## save the object
## save(scores_list, file = "~/Dropbox/Splice-n-of-1-pathways/Data/TCGA_PRAD_hel_avg_Iso30_expressiod_pathwayfilter_KEGG_25july2017.RData")
...
## save the object
save(scores_list, file = "~/Dropbox/Splice-n-of-1-pathways/Data/TCGA_PRAD_hel_EE_Iso30_expressiod_pathwayfilter_KEGG_25july2017.RData")
