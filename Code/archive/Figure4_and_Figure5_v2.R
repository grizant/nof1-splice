...
tmp_locfdr <- locfdr::locfdr(zz = transformed_pvalue, bre = ceiling(length(clust_pvalue)/8), df = 4, pct = 0, pct0 = 1/4, nulltype = 1, plot = 1, mlests = c(1,0.75))

bre = 30
mlest = c(1,0.75)
pct0 = c(1/8, 63/64)
tmp_locfdr <- locfdr::locfdr(zz = transformed_pvalue, bre = bre, df = 4, pct = 0, pct0 = pct0, nulltype = 1, plot = 1, mlests = mlest)

sum(transformed_pvalue < tmp_locfdr$z.2[1])
head(tmp_locfdr$fdr, 5)
tmp_locfdr$z.2
...
## 4 hits
## threshold <- 0.5
hits <- names(transformed_pvalue)[transformed_pvalue < tmp_locfdr$z.2[1]]
## top_hits <- names(transformed_pvalue)[tmp_locfdr$fdr < threshold]
tmp_data[hits,] ## top hits are all interesting including staph infection!
tmp_locfdr$fdr[1:length(hits)]
...
save_plot("Figure4_v2.pdf", surv_plots,
...
or_plots <- plot_grid(p1, p2, p3, p4,
                         labels=c("A", "B", "C", "D"), ncol = 2, align = "h")
save_plot("Figure5_v2.pdf", or_plots,
          ncol = 2, # we're saving a grid plot of 2 columns
          nrow = 2, # and 2 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.1,
