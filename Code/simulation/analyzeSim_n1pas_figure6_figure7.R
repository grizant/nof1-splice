...
## explore relationships with misfits
na_sim_data <- sim_data[is.na(sim_data$target_captured),]

table(na_sim_data$sim_p)
table(na_sim_data$pi)
table(na_sim_data$sim_p, na_sim_data$pi)
...
fpr_pat_data <- sim_data %>%
    filter(pi == 0) %>%
    group_by(dataset, sim_p, patientID) %>%
    summarize(mean_pat_fpr = mean(target_captured))
...
p6 <- p5 + labs(title="",x="Expressed genes in pathway", y="Average false-positive rates across patients")
...
ggsave(plot = p8, height=8, width=7, dpi=200, filename = paste0("~/Dropbox/Splice-n-of-1-pathways/Figures/CurTrendsInTransBioinfor Figures/Figure6noline_gs.pdf"), useDingbats=FALSE)
...
p1 <- p1 + geom_line(size = point_size, position = pd)
...
## alternate plot (DO NOT RUN)

point_size <- 0.5
my_width <- 0.5

## use sim_p as x axis
## p0 <- ggplot(data = power_data, aes(x = sim_p, y = mean_power, color = pi))
p0 <- ggplot(data = power_data, aes(x = factor(sim_p), y = mean_power, color = factor(pi), shape = factor(pi)))
## use pathway size as lines
## p1 <- p0 + geom_point(size=point_size, col = "black")
p1 <- p0 + geom_point(size=point_size)
## p2 <- p1 + geom_line(aes(linetype = factor(pi)), col = "black", size = point_size)
p2 <- p1 + facet_wrap(facets = vars(dataset), nrow = 3)
## p3 <- p2 + geom_errorbar(aes(ymin = lower, ymax = upper), width = my_width, col = "black", size = point_size)
p3 <- p2 + geom_errorbar(aes(ymin = lower, ymax = upper), width = my_width, size = point_size)
p4 <- p3 + scale_linetype_manual(values=c(4, 2, 3, 1), guide = guide_legend(title = "Fold Change")) 
p5 <- p4 + theme_bw(base_size = 12)
p6 <- p5 + labs(title="",x="Proportion of DEGs",y="Rejection rate")
## p7 <- p6 + theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = "none")
## p8 <- p7 + xlim(0, 0.20)
p8 <- p7 <- p6
p8

ggsave(plot = p8 ,height=8, width=7, dpi=200, filename = paste0("~/Dropbox/Splice-n-of-1-pathways/Figures/CurTrendsInTransBioinfor Figures/Figure7_alternative.pdf"), useDingbats=FALSE)
