## Score TCGA THCA patients using 2018 GOBP
...
## filter THCA iso data to just genes in pathways
## load(file = "~/Dropbox/Splice-n-of-1-pathways/Data/thca_iso_paired.RData")
## thca_iso_gobp_data <- thca_iso[which(thca_iso$geneSymbol %in% genes), ]
## str(thca_iso_gobp_data)
## tmp_genes <- unique(as.character(gobp$symbol))
## saveRDS(thca_iso_gobp_data, file = "~/Dropbox/Splice-n-of-1-pathways/Data/thca_iso_gobp_data.rds")

## load TCGA THCA TPM isoform data
thca_iso_gobp_data <- readRDS(file = "~/Dropbox/Splice-n-of-1-pathways/Data/thca_iso_gobp_data.rds") 
## DEG isoform data
...
pat_col <- grep("TCGA", x = names(thca_iso_gobp_data))
patients_chr <- unique(substring(names(thca_iso_gobp_data[pat_col]), 1, 12))
...
for (tmp_pat in patients_chr) {
    ...
    iso_gobp_list[[tmp_pat]] <- (data.frame(geneSymbol = thca_iso_gobp_data$geneSymbol,
                                            thca_iso_gobp_data[, grep(tmp_pat, names(thca_iso_gobp_data))]))
    iso_gobp_list[[tmp_pat]][, "geneSymbol"] <- as.character(thca_iso_gobp_data$geneSymbol)
}
...
## str(scores_list)
## save the object
save(scores_list, file = "~/Dropbox/Splice-n-of-1-pathways/Data/TCGA_THCA_hel_EE_Iso30_exprThreshold=5_NoDEG_GOBP_9aug2018.RData")
