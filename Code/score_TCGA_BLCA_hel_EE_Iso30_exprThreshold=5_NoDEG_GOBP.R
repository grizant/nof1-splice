## Score TCGA BLCA patients using 2018 GOBP
...
## filter BLCA iso data to just genes in pathways
## load(file = "~/Dropbox/Splice-n-of-1-pathways/Data/blca_iso_paired.RData")
## blca_iso_gobp_data <- blca_iso[which(blca_iso$geneSymbol %in% genes), ]
## str(blca_iso_gobp_data)
## tmp_genes <- unique(as.character(gobp$symbol))
## saveRDS(blca_iso_gobp_data, file = "~/Dropbox/Splice-n-of-1-pathways/Data/blca_iso_gobp_data.rds")

## load TCGA BLCA TPM isoform data
blca_iso_gobp_data <- readRDS(file = "~/Dropbox/Splice-n-of-1-pathways/Data/blca_iso_gobp_data.rds") 
## DEG isoform data
...
pat_col <- grep("TCGA", x = names(blca_iso_gobp_data))
patients_chr <- unique(substring(names(blca_iso_gobp_data[pat_col]), 1, 12))
...
for (tmp_pat in patients_chr) {
    ...
    iso_gobp_list[[tmp_pat]] <- (data.frame(geneSymbol = blca_iso_gobp_data$geneSymbol,
                                            blca_iso_gobp_data[, grep(tmp_pat, names(blca_iso_gobp_data))]))
    iso_gobp_list[[tmp_pat]][, "geneSymbol"] <- as.character(blca_iso_gobp_data$geneSymbol)
}
...
## str(scores_list)
## save the object
save(scores_list, file = "~/Dropbox/Splice-n-of-1-pathways/Data/TCGA_BLCA_hel_EE_Iso30_exprThreshold=5_NoDEG_GOBP_9aug2018.RData")
