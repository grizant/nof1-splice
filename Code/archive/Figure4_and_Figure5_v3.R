## Create Figures 4 and 5, locFDR was unstable try new attempt
## AG Schissler
## Created 27 Jul 2017
...
emp_pvalue <- sapply(clust_pvalue, function(tmp_pvalue) {
    sum(rand_pvalue < tmp_pvalue)/reps
})
...
save_plot("Figure4_v3.pdf", surv_plots,
...
or_plots <- plot_grid(p1, p2, p3, p4,
                         labels=c("A", "B", "C", "D"), ncol = 2, align = "h")
save_plot("Figure5_v3.pdf", or_plots,
          ncol = 2, # we're saving a grid plot of 2 columns
          nrow = 2, # and 2 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.1,
