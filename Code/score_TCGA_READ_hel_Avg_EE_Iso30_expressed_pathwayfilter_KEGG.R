## Score TCGA READ patients via Average pathway Hellinger distances
...
## load TCGA READ TPM isoform data
load(file = "~/Dropbox/Splice-n-of-1-pathways/Data/read_iso_kegg_data.RData") ## NEW ISOFORM 25 Jul 2017
...
pat_col <- grep("TCGA", x = names(read_iso_kegg_data))
patients_chr <- unique(substring(names(read_iso_kegg_data[pat_col]), 1, 12))
...
for (tmp_pat in patients_chr) {
    ...
    iso_kegg_list[[tmp_pat]] <- (data.frame(geneSymbol = read_iso_kegg_data$geneSymbol,
                                            read_iso_kegg_data[, grep(tmp_pat, names(read_iso_kegg_data))]))
    iso_kegg_list[[tmp_pat]][, "geneSymbol"] <- as.character(read_iso_kegg_data$geneSymbol)
}
...
## 
## ## save the object
## save(scores_list, file = "~/Dropbox/Splice-n-of-1-pathways/Data/TCGA_READ_hel_avg_Iso30_expressiod_pathwayfilter_KEGG_25july2017.RData")

## now score by Empirical Enrichment
system.time(scores_list <- parallel::parLapply(cl = cl, iso_kegg_list, transform_iso_pathway, annot_file = "~/Dropbox/Lab-Tools/GeneSets/KEGG/kegg_tb.txt", desc_file = "~/Dropbox/Lab-Tools/GeneSets/KEGG/kegg.description_tb.txt", pathway_method = "EE", gene_method = "hellinger")) ## 16 seconds

## save the object
save(scores_list, file = "~/Dropbox/Splice-n-of-1-pathways/Data/TCGA_READ_hel_EE_Iso30_expressiod_pathwayfilter_KEGG_25july2017.RData")
