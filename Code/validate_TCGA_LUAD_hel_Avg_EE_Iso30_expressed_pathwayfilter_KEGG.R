## Validate TCGA LUAD patients via Average pathway Hellinger distances
...
(target <- names(cancer_pathways)[grep("Non-small cell", cancer_pathways)])


target_data <- get_target_info(scores_list, target = target, cancer_ids = cancer_ids, fdr = 0.2, one_sided = T)
target_data <- get_target_info(scores_list, target = target, cancer_ids = cancer_ids, fdr = 0.2, one_sided = F)
## target_data <- get_target_info(scores_list, target = target, cancer_ids = cancer_ids, fdr = 0.5, one_sided = T)
